...
public class LoginCus extends Stage {
    ...
    private void checkCredentials(Consumer<Boolean> loginResult) throws SQLException {
        if (!email.getText().matches("^[A-Za-z0-9+_.-]+@(.+)$")) {
            error.setText("Incorrect email format");
            loginResult.accept(false);
        }
        this.userDao.tryLogin(this.email.getText(), this.password.getText(), result -> {
            if (!result) {
                error.setText("Incorrect password or email");
            } else {
                loginResult.accept(result);
                error.setText("");
            }
        });
    }
    ...
    private void createScene() {
        ...
        anchorpane.setPrefHeight(FxApplication.SCENE_WIDTH);
        anchorpane.setPrefWidth(FxApplication.SCENE_WIDTH);
        Pane pane = new Pane();
        pane.setPrefHeight(FxApplication.SCENE_WIDTH);
        pane.setPrefWidth(FxApplication.SCENE_WIDTH);

...
public class CreateCustomer extends Stage {
    ...
    public CreateCustomer(CustomerDao userDao) {
    ...
    private void closeStage() {
        ...
        if (passwordfield.getText().isEmpty() || passwordfield2.getText().isEmpty() || !FxApplication.equals(passwordfield.getText().toCharArray(), passwordfield2.getText().toCharArray())) {
        ...
        for (TextField field : fields) {
            switch (field.getPromptText().toLowerCase()) {
                ...
                case "example@example.com":
                    email = field.getText();
                    this.customerDao.profileExists(email, result -> {
                        if (result || !field.getText().matches("^[A-Za-z0-9+_.-]+@(.+)$")) {
                            redOutField(field);
                            errors = true;
                        }
                    });
        ...
        if (!errors) {
            this.customerDao.createPerson(email, pass, ssn, name, sname, addr, phone);
